...
func TestDirectGatewayRequiresIdentityOnceAnyKeyExists(t *testing.T) {
	fakes := newGatewayFakes()
	access := &fakeGatewayAccess{}
	system, err := NewSystem(Config{Addr: "127.0.0.1:0", Access: access}, fakes.runtime, fakes.roles, fakes.groups, fakes.workspaces, fakes.providers, fakes.tools, fakes.sessions, fakes.stickers, fakes.hooks, fakes.placeholders, fakes.systemPlugins, fakes.assist, fakes.releaseSource, fakes.requestRecords)
	if err != nil {
		t.Fatalf("NewSystem() error = %v", err)
	}
	rec := httptest.NewRecorder()
