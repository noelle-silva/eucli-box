...
func TestSessionMessagePartsAreNormalized(t *testing.T) {
	...
	session := types.Session{
		...
		Messages: []types.Message{{
			...
			CreatedAt: now,
			UpdatedAt: now,
			Parts:     []types.MessagePart{{Type: "tool", Raw: `{"command":"pwd"}`, CallID: "call-1", ToolName: "shell_command", State: "completed", Input: map[string]any{"command": "pwd"}, Result: &types.ToolPartResult{ID: "result-1", ActionID: "call-1", ToolName: "shell_command", Status: types.ToolStatusSuccess, Content: "ok"}}},
		}},
		LastActive: now,
	}
	if err := system.SaveSession(context.Background(), session); err != nil {
	...
	if loaded.Messages[0].Parts[1].Type != "tool" || loaded.Messages[0].Parts[1].Raw == "" || loaded.Messages[0].Parts[1].CallID != "call-1" || loaded.Messages[0].Parts[1].Result == nil {
