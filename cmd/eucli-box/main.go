...
func run() error {
	...
	log.Printf("[1/10] network-request-system ✓")
	...
	log.Printf("[2/10] data-storage-system     ✓  (%s)", dataDir)
	...
	log.Printf("[3/10] model-provider-system   ✓")
	...
	log.Printf("[6/10] tool-calling-system     ✓")

	placeholderSystem, err := placeholdersystem.NewSystem(placeholdersystem.Config{RootDir: dataDir})
	if err != nil {
		return fmt.Errorf("start placeholder system: %w", err)
	}
	log.Printf("[7/10] placeholder-system      ✓")
	...
	log.Printf("[8/10] agent-runtime-system    ✓")
	...
	log.Printf("[9/10] ai-assist-system        ✓")

	busyKey := ""
	if readBoxKey(dataDir) != "" {
		busyKey = " (key: active)"
	}
	gatewaySystem, err := gateway.NewSystem(gateway.Config{Addr: envOrDefault("EUCLI_BOX_ADDR", "127.0.0.1:8765"), Key: readBoxKey(dataDir)}, runtimeSystem, roleSystem, storageSystem, storageSystem, providerSystem, toolSystem, storageSystem, storageSystem, storageSystem, placeholderSystem, assistSystem)
	if err != nil {
		return fmt.Errorf("start gateway system: %w", err)
	}
	log.Printf("[10/10] gateway-system         ✓%s", busyKey)
