...
// Stage04 阶段四验证：工具与插件首次安装和手动更新。
func Stage04(ctx context.Context, repositoryRoot string, runRoot string, mode string) error {
	paths, err := prepareRun(repositoryRoot, runRoot, "04")
	if err != nil {
		return err
	}
	...
	if mode != "default" && mode != "experience" {
		return fmt.Errorf("阶段四模式必须是 default 或 experience")
	}
	recorder := newRecorder("04", mode, paths.root)
	fmt.Printf("阶段四 %s 验证目录：%s\n", mode, paths.root)
	...
	boxPath, err := prepareStage04Box(ctx, repositoryRoot, paths, recorder)
	if err == nil {
		runStage04Verification(ctx, repositoryRoot, paths, recorder, boxPath)
	}
	...
	if dataErr == nil {
		...
		if snapshotErr != nil {
			...
			recorder.pass("确认真实数据未改变", "阶段四验证未写入真实 data 目录")
		}
	}
	if gitErr == nil {
		...
		if snapshotErr != nil {
			...
			recorder.pass("确认源码未被验证改写", "阶段四验证只在本次隔离目录产生运行内容")
		}
	}
	return recorder.finish(paths)
}

// prepareStage04Box 构建隔离业务端可执行文件供验证子进程使用。
func prepareStage04Box(ctx context.Context, root string, paths runPaths, recorder *recorder) (string, error) {
...
func runStage04Verification(ctx context.Context, root string, paths runPaths, recorder *recorder, boxPath string) {
	runTest := "^TestStage04$"
	if strings.TrimSpace(os.Getenv("EUCLI_STAGE04_MODE")) == "experience" {
		runTest = "^TestStage04Experience$"
	}
	command := struct {
		name    string
		workdir string
		...
		workdir: root,
		command: "go",
		args:    []string{"test", "-tags", "eucli_stage04", "-run", runTest, "-count=1", "devtools/general-verification-tools/verify-tool-plugin-update/stage04verify"},
		env: map[string]string{
			"EUCLI_STAGE04_RUN_ROOT": paths.root,
			"EUCLI_STAGE04_BOX":      boxPath,
			"EUCLI_STAGE04_MODE":     os.Getenv("EUCLI_STAGE04_MODE"),
		},
	}
