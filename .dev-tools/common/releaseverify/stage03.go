...
func prepareVerificationBox(ctx context.Context, root string, paths runPaths, name string, outputRoot string, recorder *recorder) (string, string, error) {
	...
	buildCommand := []string{"run", "devtools/eucli-release", "build", "-root", root, "-target", "eucli-box", "-work-root", filepath.Join(paths.workspace, "box-build"), "-output-root", outputRoot, "-evidence-root", filepath.Join(paths.evidence, "box-build"), "-asset-root", workspace.VerificationAssetRoot(root), "-verification-only", "-result-file", resultPath}
