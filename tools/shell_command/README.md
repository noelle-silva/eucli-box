...
工具定义声明了「定时预热」开关（默认开启，可在用户配置中关闭）。开启后，宿主会按自己的节奏向工具发送专用预热请求，工具收到后会加载命令分析器，并拉起默认 Provider 的 shell 执行一条固定无害命令（`exit 0`）；这只用于保持系统对工具文件的扫描缓存热度，减少长时间未用后首次调用的等待。

- 预热请求不执行任何用户命令，不产生工具调用记录。
- 预热会真实加载分析器与默认 Provider，但不影响用户命令的执行流程与结果。
- 关闭后宿主不再预热本工具。
